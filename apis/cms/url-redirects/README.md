# Go API client for url-redirects
...
```golang
import url-redirects "github.com/pyscht/hubspot-go-api/apis/cms/url-redirects"
```
...
```golang
ctx := context.WithValue(context.Background(), url-redirects.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), url-redirects.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), url-redirects.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), url-redirects.ContextOperationServerVariables, map[string]map[string]string{
